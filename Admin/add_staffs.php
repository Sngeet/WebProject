...
if (isset($_POST['submit'])) {
    ...
    if ($fetchData['cnt'] > 0) {
        echo "<script>alert('Email or phone number already exists');
            window.location = 'add_staffs.php';
        </script>";
    } else {
        $qryReg = "INSERT INTO `staff`(`sname`,`semail`,`sphone`,`saddress`) VALUES('$Name','$Email','$Phone','$Address')";
        $qryLog = "INSERT INTO `login` (`rid`, `email`, `password`, `type`) VALUES((SELECT MAX(`sid`) FROM `staff`),'$Email', '$hashed_password','Staff')";

        if ($conn->query($qryReg) === TRUE && $conn->query($qryLog) === TRUE) {
            echo "<script>alert('New Staff Added Successfully'); window.location = './add_staffs.php';</script>";
        } else {
            echo "<script>alert('Registration Failed'); window.location = './add_staffs.php';</script>";
        }
    }
